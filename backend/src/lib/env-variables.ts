...
export const DATABASE_URL = getKubeConfig(
	process.env.DATABASE_URL,
	'postgresql://app_fec:FECTemp@12345@192.168.55.245:5432/synapses_db?sslmode=disable'
);

// Keycloak Configuration (from Kubernetes in production)
export const KEYCLOAK_URL = getKubeConfig(process.env.KEYCLOAK_URL, '');
export const KEYCLOAK_CLIENT_REALM = getKubeConfig(
	process.env.KEYCLOAK_CLIENT_REALM,
	''
);
export const KEYCLOAK_CLIENT_ID = getKubeConfig(
	process.env.KEYCLOAK_CLIENT_ID,
	''
);

// Secrets (from Kubernetes Secrets in production)
export const KEYCLOAK_CLIENT_SECRET = getSecret(
	process.env.KEYCLOAK_CLIENT_SECRET
);
export const SESSION_SECRET = getSecret(
...
export const GITHUB_INSTALLATION_ID = getKubeConfig(
...
export const GITHUB_PRIVATE_KEY_PATH = getSecret(
...
export const AI_API_URL = getConfig(
	process.env.AI_API_URL,
	'http://localhost:11434'
); // Ollama default or remote server
export const AI_MODEL_NAME = getConfig(process.env.AI_MODEL_NAME, 'phi3');
export const AI_ENABLE_MCP_TOOLS =
	getConfig(process.env.AI_ENABLE_MCP_TOOLS, 'true') !== 'false'; // Default true
export const AI_USE_SIMPLE_MODE =
	getConfig(process.env.AI_USE_SIMPLE_MODE, 'false') === 'true'; // Default false
