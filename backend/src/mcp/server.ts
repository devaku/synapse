  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'create_task':
            result = await TaskHandlers.createTask(args, this.userId);
            break;

          case 'read_tasks':
            result = await TaskHandlers.readTasks(args, this.userId);
            break;

          case 'update_task':
            result = await TaskHandlers.updateTask(args, this.userId);
            break;

          case 'subscribe_task':
            result = await TaskHandlers.subscribeTask(args, this.userId);
            break;

          case 'unsubscribe_task':
            result = await TaskHandlers.unsubscribeTask(args, this.userId);
            break;
