...
export async function subscribe(req: Request, res: Response) {
  try {
    ...
    if (!userId) {
      return res.status(401).json(buildError(401, 'Unauthorized - No valid session', null))
    } // Idk if we need validation for this

    if (!taskId || isNaN(taskId)) {
      return res.status(400).json(buildError(400, 'Invalid task ID', null))
    ...
    if (existingSubscription) {
      return res
        .status(409)
        .json(buildError(409, 'User is already subscribed to this task', null))
    }
    ...
    return res
      .status(200)
      .json(buildResponse(200, 'User subscribed to task successfully.', null))
  } catch (error: any) {
    console.error('SUBSCRIBE ERROR:', error)
    return res.status(500).json(buildError(500, 'Error subscribing to task', error.message || 'Unknown error'))
  }
}
...
export async function unsubscribe(req: Request, res: Response) {
  try {
    ...
    const unsub = await subscriptionService.unsubscribeFromTask(userId, taskId)
    return res.status(200).json(buildResponse(200, 'Unsubscribed from task', unsub))
  } catch (error: any) {
    console.error('UNSUBSCRIBE ERROR:', error)
    
    // Handle the case where they try to unsubscribe from something they weren't subscribed to
    if (error.code === 'P2025') {
      return res.status(404).json(buildError(404, 'Subscription not found', null))
    }
    return res.status(500).json(buildError(500, 'Error unsubscribing from task', error.message || 'Unknown error'))
  }
}
...
export async function listSubscribedTasks(req: Request, res: Response) {
  try {
    ...
    return res.status(200).json(buildResponse(200, 'Subscribed tasks retrieved', tasks))
  } catch (error: any) {
    console.error('LIST SUBSCRIPTIONS ERROR:', error)
    return res.status(500).json(buildError(500, 'Error retrieving subscribed tasks', error.message || 'Unknown error'))
  }
}