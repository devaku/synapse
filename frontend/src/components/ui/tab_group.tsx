...
export default function TabGroup({ tabs, style = TabGroupStyle.Default }: { tabs: Array<Tab>; style?: TabGroupStyle }) {
    const [activeTab, setActiveTab] = useState(tabs[0].name);

    function Tab({ name, selected = false, onClick }: Tab) {
		return (
			<button
				className={styles[style].base + ' ' + (selected == true ? styles[style].selected : styles[style].unselected)}
				onClick={onClick}
			>
				{name}
			</button>
