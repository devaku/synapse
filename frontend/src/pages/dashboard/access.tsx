...
export default function MyAccesssPage() {
	...
	const [showModalAccessInfo, setShowModalAccessInfo] = useState<boolean>(false);
	...
	return (
		<main className="flex flex-row h-screen w-full">
			...
			<SlideModalContainer isOpen={showModalAccessInfo} noFade={false}>
				<div>
					<h1>{findTableEntryById(modalAccessId, mockAccessAPIResponse)?.repo_name}</h1>
					<p>{findTableEntryById(modalAccessId, mockAccessAPIResponse)?.access_requested}</p>
					<p>{findTableEntryById(modalAccessId, mockAccessAPIResponse)?.requesting_user}</p>
					<button onClick={handleModalAccessInfoDisplay}>Close</button>
				</div>
			</SlideModalContainer>
		</main>
	);
